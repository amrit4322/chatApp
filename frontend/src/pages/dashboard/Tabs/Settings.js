...
const Settings = () => {
  ...
  const handleUpload = async (e) => {
    ...
    if (selected) {
      ...
      const response = await apiInstance.uploadFile(
        "/user/uploadFile",
        formData,
        token
      );
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await apiInstance.patchWithToken(
      "/user/update",
      formData,
      token
    );
            ...
            <div className="mb-4 profile-user">
              {currentImage ? (
                <div className="">
                  <img
                    src={
                      selected ? URL.createObjectURL(selected) : currentImage
                    }
                ...
                >
                  <div
                    className="avatar-lg d-flex align-items-center justify-content-center rounded-circle bg-soft-primary text-primary"
                    onClick={handleClickProfilePic}
                    style={{ cursor: "pointer" }}
                  >
